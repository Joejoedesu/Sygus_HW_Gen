...
((Start (_ FloatingPoint 8 24)
   (
            ...
            ;  (fp.add RNE Start Start) ; constraint linear approximation
   ...
   (
            ...
            (ite CaseBool Const Const) ; not necessary
            ))
   (CaseBool Bool 
   (        ( fp.lt Start Start) ; leq
    			(= Start Start)
...
(constraint (=> (and (= x ((_ to_fp 8 24) RNE r)) (<= r 2.0) (>= r -2.0))
                (fp.lt (fp.abs (fp.sub RNE (f x)
                 ((_ to_fp 8 24) RNE (sin r)))) ((_ to_fp 8 24) RNE 0.1))))
